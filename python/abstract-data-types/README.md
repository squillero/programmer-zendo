...
- An Updatable Priority Queue (using lazy-deletion heap)
- A Radix Tree
- Some Graph alternative implementations

```shell
# BST
uv run pytest -v test-bst.py
# TRIE
uv run pytest -v test-trie.py
# Graph
uv run visits.py
uv run alcuinus.py
```
...
Pure Python implementation of a [*radix trie*](https://en.wikipedia.org/wiki/Radix_tree), a.k.a., *radix tree* or *compact prefix tree* or *compressed trie*.
...
and then the radix trie would be something like
